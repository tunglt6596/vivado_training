...
Xuint8 carrier_hdmi_out_config[CARRIER_HDMI_OUT_CONFIG_LEN][3] =
{
		...
		{ADV7511_ADDR>>1, 0xAF, 0x06}, // HDMI/DVI Modes 04
...
int HDMIWriteReg(u8 SlaveAddr, u8 u8RegAddr, u8 u8RegVal)
{
	int i;
	...
	do
	{
		StatusReg = Xil_In8(XPAR_HDMI_DISPLAY_AXI_IIC_0_BASEADDR + XIIC_SR_REG_OFFSET);
	...
	SentByteCount = XIic_DynSend((u32) XPAR_HDMI_DISPLAY_AXI_IIC_0_BASEADDR, SlaveAddr, &pI2cDataSend[0], (unsigned) 2, (u8) XIIC_STOP);
	if(SentByteCount < 1)
	{
		xil_printf("Write 7194 register failed\r\n");
...
void InitDisplayHDMI()
{
	int i, k;

	if(XIic_DynInit(XPAR_HDMI_DISPLAY_AXI_IIC_0_BASEADDR) == -1)
	{
	...
	for ( k = 0; k < 100; k++ )
	{
		for ( i = 0; i < CARRIER_HDMI_OUT_CONFIG_LEN; i++ )
		{
			...
			HDMIWriteReg( carrier_hdmi_out_config[i][0], carrier_hdmi_out_config[i][1], carrier_hdmi_out_config[i][2]);
		}
	}
}
